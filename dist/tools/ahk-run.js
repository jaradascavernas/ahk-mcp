...
export class AhkRunTool {
    async detectWindow(pid, options) {
    ...
    static async findAutoHotkeyPath() {
    ...
    runOnce(ahkExe, scriptPath, options) {
    ...
    async execute(args) {
        try {
            ...
            if (mode === 'run') {
                ...
                return {
                    content: [
                        { type: 'text', text: `AHK launched: ${file}` },
                        { type: 'text', text: response.windowDetected ? `✅ Window detected: ${response.windowInfo?.title}` : (detectWindow ? '⏳ No window detected' : '') },
            ...
            if (!enabled) {
                ...
                return { content: [{ type: 'text', text: 'AHK watch disabled and running processes cleaned up.' }] };
            }
            const file = await this.ensureFile(filePath);
            ...
            try {
                ...
                return {
                    content: [
                        { type: 'text', text: `Watching for edits: ${file}` },
                        { type: 'text', text: `AutoHotkey path: ${resolvedAhkPath}` },
                        { type: 'text', text: `Runner: ${runner}, Kill on exit: ${killOnExit}` }
                    ]
                };
            }
            catch (watchErr) {
