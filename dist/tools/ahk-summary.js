    ...
    async execute() {
        ...
        if (fullDocs && fullDocs.data) {
            ...
            if (fullDocs.data.BuiltInVariables) {
                variables = fullDocs.data.BuiltInVariables.map((v) => ({
                    name: v.Name,
            ...
            if (fullDocs.data.Classes) {
                const classItems = fullDocs.data.Classes.filter((item) => item.Type === 'Class' || !item.Path);
                const methodItems = fullDocs.data.Classes.filter((item) => item.Type === 'Method' && item.Path);
                classes = classItems.map((c) => ({
                    name: c.Name,
                    description: c.Description || 'No description available'
                }));
                methods = methodItems.map((m) => ({
                    name: `${m.Path}.${m.Name}`,
