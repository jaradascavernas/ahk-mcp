...
function includeBySeverity(name, filter) {
    if (filter === 'all')
        return true;
    if (filter === 'error')
        return name === 'Error';
    if (filter === 'warning')
        return name === 'Warning';
    if (filter === 'info')
        return name === 'Information' || name === 'Hint';
    return true;
}
export class AhkVSCodeProblemsTool {
    async execute(args) {
        try {
            ...
            const filtered = normalized.filter(n => includeBySeverity(n.severityName, severity) &&
