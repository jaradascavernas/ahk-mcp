...
interface CompletionContext {
  type: 'function-call' | 'variable-access' | 'class-member' | 'directive' | 'hotkey' | 'global';
  prefix: string;
  className?: string;
  line: string;
  position: Position;
...
export class AhkCompletionProvider {
   ...
   */
  private analyzeContext(code: string, position: Position): CompletionContext {
  ...
  private isHotkeyContext(line: string): boolean {
    return line.includes('::') && !line.trim().startsWith(';');
  }

  private isFunctionCallContext(beforeCursor: string): boolean {
    // Check if we're in a function call context
    return /\w+\s*\(\s*\w*$/.test(beforeCursor);
  }

  private isVariableContext(beforeCursor: string): boolean {
   ...
   */
  private getVariableCompletions(prefix: string): CompletionItem[] {
   ...
   */
  private getClassMemberCompletions(className: string, prefix: string): CompletionItem[] {
    ...
    // Find the class
    const ahkClass = index.classes.find(cls => 
   ...
   */
  private getDirectiveCompletions(prefix: string): CompletionItem[] {
    const index = getAhkIndex();
    if (!index) return [];
    
    return index.directives
      .filter(directive => 
        directive.Name.toLowerCase().includes(prefix.toLowerCase())
      )
      .map(directive => ({
   ...
   */
  private getHotkeyCompletions(prefix: string): CompletionItem[] {
    const commonHotkeys = [
    ...
    return commonHotkeys
      .filter(hotkey => 
        hotkey.key.toLowerCase().includes(prefix.toLowerCase())
      )
      .map(hotkey => ({
        label: hotkey.key,
        kind: CompletionItemKind.Keyword,
   ...
   */
  private getGlobalCompletions(prefix: string): CompletionItem[] {
    const completions: CompletionItem[] = [];
   ...
   */
  private formatFunctionInsertText(func: any): string {
    if (!func.Parameters || func.Parameters.length === 0) {
      return `${func.Name}()`;
    }
    
    const params = func.Parameters.map((param: any, index: number) => {
   ...
   */
  private formatMethodInsertText(method: any): string {
    if (!method.Parameters || method.Parameters.length === 0) {
      return `${method.Name}()`;
    }
    
    const params = method.Parameters.map((param: any, index: number) => {
   ...
   */
  private formatMethodDocumentation(method: any): string {
